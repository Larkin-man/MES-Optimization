...
int TBaseForm::DjonsonAlgorithm()
{
	...
	TimeCycleMethod[MDJ] = Optimizer->DjonsonRun(); //Запуск алгоритма Петрова-Соколицына
	if (out)
	{
...
{
	//StatusBar1->Panels->Items[0]->Text=("Метод Петрова-Соколицына");
	TimeCycleMethod[MPS] = Optimizer->PetrovSokolRun(); //Запуск алгоритма Петрова-Соколицына
	top = new int[N];
